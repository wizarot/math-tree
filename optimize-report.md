...
| `assets/left.png`→`assets/left.webp` | 1184×3588 | 512×1552 (WebP) | 7852 KB → **76 KB** |
...
| 项目 | 优化前 | 优化后 |
|------|--------|--------|
| 四大 PNG | ~21.4 MB | ~1.1 MB |
| `math-topics.json` | 917 KB | 741 KB |
| `mtf-sprite.svg` + `math-dependencies.json` | ~792 KB | ~792 KB（结构未动） |
| **合计关键传输量** | **≈ 23.2 MB** | **≈ 2.9 MB（↓约 87%）** |

> **修订（2026-08-18）**：
> - **compass** 最初压到 256 在 2x 屏（绘制 136px→272 设备像素）显糊，已回滚后以原图重压到 1024×1024（1000KB）。中心罗盘为焦点元素且有精细刻度文字，1024 对 136px@2x 有 3.7x 超采样余量，清晰度足够；体积仍比原始 3346KB 小 3.3x。原始 2048 大图保留在 `bak/heavy_originals/compass.png`。
> - **left（侧边栏底图）** 原 PNG 1100KB（且 RGBA 全不透明，alpha 通道浪费）。改为**同分辨率 512 宽转 WebP（质量 90）**，清晰度与 PNG 完全一致，体积仅 **76KB**（↓约 14 倍）；原 `left.png` 移出 `assets/` 至 `unused-assets/`。静态服务器实测 `Content-Type: image/webp` 正常返回。如部署环境不支持 WebP，可回退 PNG 512（1100KB）。
...
3. `left.png` 已转 **WebP**（512 宽 76KB），清晰度不变、体积降约 14 倍；若部署环境不支持 WebP，可回退 PNG 512（1100KB）或降到 384 宽（≈600KB，金边细节会更软，但侧边栏金边由 CSS 兜底）。
